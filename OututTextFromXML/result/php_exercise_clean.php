...
Class LocationsAndTimes {
    ...
    private function formatDaysAndHours(array $daysAndTimes) {
    ...
    //find all the days that have this time schedule
        foreach($uniqueTimes as $hours) {
                ...
                sort($daysWithTheseHours);  //make sure we are in order
                $processedDays = 0;
                $combinedDays =[];
                foreach($daysWithTheseHours as $key =>  $day) {
                    ...
                    if(in_array($tomorrow, $daysWithTheseHours)) {
                        ...
                        while (in_array($tomorrow, $daysWithTheseHours)) {
                         ...
                         $combinedDays[] = $this->days[$day] .'-'. $this->days[$day+$consectiveDays] ;
                         $processedDays = $processedDays + $consectiveDays + 1 ;
                    }  else {
                        $processedDays = $processedDays + 1;
                        $combinedDays[] = $this->days[$day] ;
                    } 
                    //we might have process the whole week at once so check if we are done
            ...
            */
            ...
            //MAKE SURE WE ALWAYS START WITH SUNDAY
            ...
            //MAKE SURE WE ALWAYS END WITH SATURDAY
            foreach ($combinedDaysAndTimes as $key => $hoursGroups) {
    ...
    private function buildUniqueTimeGroupsFromLocation($location) {
        $daysAndTimes = [];
        foreach($location['schedule'] as $period) {
    ...
    public function process() {
        foreach($this->locations as $location) {
            $this->output[] = '#'.$location['biz_id']. ': ' .trim($location['biz_name']) . ' | ' . $this->formatDaysAndHours($this->buildUniqueTimeGroupsFromLocation($location));
        }
        return $this;
    }
...
echo (new LocationsAndTimes)->setLocations($restaurants['response']['data'])->process()->outputText();